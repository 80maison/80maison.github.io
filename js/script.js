...
if(contactViewportTop < window.innerHeight && contactViewportBottom > 0){
const fadeStart = window.innerHeight;
const fadeDistance = window.innerHeight * CONTACT_FADE_SPAN;
const contactProgress = Math.min(1,Math.max(0,(fadeStart - contactViewportTop)/ fadeDistance));
const parallaxProgress = Math.max(0,window.innerHeight - contactViewportTop);
...
window.addEventListener('resize',()=>{measureLayout();requestScrollUpdate();},{passive:true});
